...
public class Learn{
    ...
    public static void main(String[] args){
        ...
        LearningEvaluator le =  new LearningEvaluator("weights/alpha.csv", .1);
        BaseEvaluator be = new BaseEvaluator("weights/beta.csv");
        ...
        for(int i = 0; i < num_games; i++){ // play num_games amount of games
            ...
            if(played == 10){
                if(won >= 7){ // if alpha wins 7 of every ten games, make beta use alpha's new evaluator
                    le.commitWeights("weights/beta.csv");
                    be.refreshWeights();
                }
                played = 0;
                won = 0;
    ...
    public static int play(CheckersGameState s, CheckersAI alpha, CheckersAI beta, LearningEvaluator le){
        ...
        while(!current.isTerminal() && moves <= 50){
            ...
            current.printState();
            moves++;
            if(current.isTerminal()){ // if alpha won, then brea
                break;
            }
